...
export default function GenerarPage() {
  ...
  const handleGenerarRutina = async () => {
    ...
    try {
      const data = await generarRutina({
  ...
  const copiarRutina = async () => {
    ...
    let texto = `${rutinaGenerada.nombre}\n`;
    texto += `Fecha: ${rutinaGenerada.fecha}\n`;
    texto += `Grupos musculares: ${rutinaGenerada.grupos_musculares.join(', ')}\n\n`;
    texto += 'Ejercicios:\n';
    rutinaGenerada.ejercicios.forEach((ej, i) => {
  ...
  const guardarSinIniciar = async () => {
    if (!rutinaGenerada) return;
    
    try {
      await crearEntrenamiento(rutinaGenerada);
      alert('¡Rutina guardada en MongoDB!');
    } catch (err) {
            ...
            <div className="bg-gym-dark/50 rounded-xl border border-white/10 p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ej: Quiero enfocarme en la parte superior del pecho..."
              ...
              {rutinaGenerada ? (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    disabled={iniciando}
                    className="w-full py-4 rounded-lg bg-gradient-to-r from-gym-green to-green-500 hover:opacity-90 transition-colors flex items-center justify-center gap-2 text-white font-semibold disabled:opacity-50"
                  >
                    {iniciando ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                    ...
                    {iniciando ? 'Iniciando...' : '¡Iniciar Entrenamiento!'}
                  </button>
                </div>
              ) : (
